...
const SUITES = [
  ...
  ["klyfton", "Queen router pure exports: shouldSkipSynth time-budget guard, bestAnswer (fullest worker answer), routerToolHint LIVE/OFF status, toolBagBlock, assembleBrainBlocks, isActionCommand (pure-command bypass), applyMemoryContext (memory-warm enrichment) — no network/no synth"],
  ...
  ["job-phases", "return-trip planner: bid scopes → the ordered TRIPS MGSF actually makes (under-slab → foundation → rim → walls → attic, plus standalone roofing/coatings/lifting/retrofit); ONE mobilization per trip however many scopes ride on it, job minimum enforced PER TRIP not per bid, unphased scopes bill exactly as before; all money supplied by the caller, none invented"],
