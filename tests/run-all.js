...
const SUITES = [
  ...
  ["doctrine", "locked doctrine single-source (mgsf-core v2 2026-08-05): pinned R-values/yields/costs/labor + a DRIFT GUARD that spf-takeoff & rvalue-calc read R-values from doctrine (no re-run of the 6.5-vs-7.1-vs-7.0 split)"],
  ...
  ["break-even", "overhead-recovery break-even (Finance/Owner-Strategy): contribution-margin math, round-up units, target-profit extension, honest no-break-even when variable cost ≥ price, unit-agnostic, guards — all figures owner-entered"],
  ...
  ["ops-manager", "the foreman over the doer agents: supervise() ranks the crew's next actions by revenue/risk (AR > speed-to-lead > cooling bid > compliance-halt > throughput), surfaces dark-tool blockers as escalations, derives each agent's station, rolls one owner summary + approve-queue; pure/deterministic, recommend-only (arms gate still applies), never auto-sends/fabricates"],
