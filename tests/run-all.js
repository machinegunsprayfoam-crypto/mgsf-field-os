...
const SUITES = [
  ...
  ["klyfton", "Queen router pure exports: shouldSkipSynth time-budget guard, bestAnswer (fullest worker answer), routerToolHint LIVE/OFF status, toolBagBlock, assembleBrainBlocks, isActionCommand (pure-command bypass), applyMemoryContext (memory-warm enrichment), matchCombo/convenePlan (cube overlap teams) — no network/no synth"],
  ["combos", "the cube's capability algebra: 6 faces + 12 edges + 8 corners = 26 pieces enumerated, axis/adjacency rules, featured overrides vs auto-generated suggestions, router fast-path + convene-by-key, and the drift guard vs klyfton SPECIALISTS"],
  ["calendar", ".ics generator (Scheduling/Dispatch's calendar tool): valid iCalendar structure, all-day vs timed + default end, the SUNDAY hard refusal (family-time doctrine), RFC5545 escaping, input guards — keyless, deterministic"],
  ["samgov", "SAM.gov pure mappers (GovCon's lead pipeline): normalize (raw notice → clean shape, primary-POC preference, city/ST place, id fallback, safe on missing) + oppToLead (clean opp → Government lead card, joined notes, value 0 never fabricated) — no network"],
  ["labor-burden", "base wage → fully-loaded hourly cost (feeds job-cost laborRate): only statutory FICA baked in, workers'-comp/FUTA/benefits owner-entered (never invented), understated-flag when comp/benefits omitted, burden math + guards"],
