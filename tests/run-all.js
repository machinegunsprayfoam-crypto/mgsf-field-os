...
const SUITES = [
  ...
  ["drive", "Google Drive proxy: Apps Script exec-URL guard (relay-safe), payload sanitization (webappUrl stripped), dormant path, token forwarding, redirect:follow for 302 — no npm deps"],
  ["tts", "TTS proxy: provider detection (ElevenLabs > OpenAI), client-key sk- sniff, voice lists, text cap, dormant/configured paths, audio/mpeg + no-store response, no npm deps"],
  ["notify", "event-webhook proxy: lineFor() message templates (new_lead/job_scheduled/job_completed/invoice/review_ask/reorder), _hash idempotency key, secret injection, SMS guard, no npm deps"],
