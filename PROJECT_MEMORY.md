...
- **★ INTELLIGENCE ENGINES (2026-08-06, on branch/PR #89, gate 101 suites / 2473) — from Clifton's Foam-Guru-parity vision. Pattern: tested pure backend cores; UIs follow.**
    ...
    - **REMAINING Yield-cluster gaps (frontend/data-model):** estimator multi-area UI (add-a-row, Clifton chose), rig-side session log (substrate temp/moisture, sets, pressure/tip, DRUM LOT #, GUN HOURS, spray-session timeline), crew/rig scorecards, same-day margin-leak alert, true offline (Service Worker + IndexedDB), hands-free voice logging, Field Mode.
