...
- **VEHICLE ARCHITECTURE** ([`VEHICLE_ARCHITECTURE.md`](VEHICLE_ARCHITECTURE.md)) — Clifton's frame: the whole system is a car (engine=hive, transmission=gearbox, clutch=arms/approval, driveshaft=event spine, differential=Queen router, dashboard=Command Center, battery=memory, brakes=doctrine gates, wheels=outward actions). **★ Two prime movers:** the ENGINE (event-driven/reactive) and the **AXLE that drives TIME** (the scheduler — daily brief, sweeps, cert-expiry, SAM scans turn the same drivetrain on a cadence). Next build: the axle = a time-driven `turn()` that engages a saved transmission program on a schedule.
