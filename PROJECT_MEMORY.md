...
- **★ HEARTH BRIDGE — BOTH receivers BUILT (2026-08-06, on `claude/klyfton-ai-problems-ynhx9f`, not merged).** Clifton runs TWO separate "Hearth" things — keep them straight: (1) an **AI Receptionist** for phone calls (number **(605) 349-2884**; notifications route to email + SMS only — no webhook field seen; the numbers 406-941-2428 / 406-550-3408 / 406-939-8301 + clifton@ / daniel@ are already in its Company Notification Preferences); (2) **Hearth financing** (gethearth.com) — customer prequal link, emails owner from **noreply@gethearth.com**. NOTE: this supersedes/clarifies the TRANSMISSION_COUPLING spec's "first coupling = Hearth call.missed" — the phone side is the *receptionist*, not gethearth.
    ...
    - **PENDING (owner):** (a) Vercel **Protection Bypass for Automation** secret (or turn Deployment Protection off) — else Zapier's POST hits the 403 wall; (b) add the parser email to the receptionist's Notification Emails; (c) set `ALERTS_WEBHOOK_URL` (or send SMS/CRM directly in the Zap). **NEED FROM CLIFTON:** a real sample email from `noreply@gethearth.com` to tune Bridge B's field aliases (searched the connected inbox 2026-08-06 — none present yet; his signature already carries the financing line). Gate at time of build: **94 suites / 2246 checks** green.
