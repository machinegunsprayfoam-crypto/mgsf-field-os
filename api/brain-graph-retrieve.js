...
const CLUSTER_BLOCKS = {
  "Foam Verification":  ["FOAM_SPECS", "DOCTRINE", "SERVICE_ARCHITECTURE", "MASTERY", "SAFETY_OSHA", "SALES_OBJECTIONS", "WARRANTY_CALLBACK", "DISCOVERY_QUALIFY"],   // foam/spray/doctrine/spec/ahj/verify + isocyanate/MDI/re-occupancy is SPF-specific safety + the DIY/off-gassing/safety objection rebuttals + a foam callback (gap/leak) needs FOAM_SPECS' diagnosis alongside the triage playbook + a first-call foam symptom (drafty/cold/condensation) needs DISCOVERY_QUALIFY's diagnostic questions before FOAM_SPECS gets applied
  "Business Licensing": ["BUSINESS", "FEDERAL", "DOCTRINE", "PROCUREMENT", "CREDENTIAL_MAP", "PROOF_ECONOMICS", "SALES_OBJECTIONS", "WARRANTY_CALLBACK", "DISCOVERY_QUALIFY"], // license/insurance/sam/contractor — credentials + gov + credential→service map (gap #1) + gov specs that mandate testing (proof pass) + the "DIY it" objection leans on CREDENTIAL_MAP + "warranty" already aliases here (manufacturer-warranty applicator gate) so a warranty-coverage question also needs WARRANTY_CALLBACK's scope-of-coverage reasoning + BUSINESS carries the MT/ND/SD/WY territory + FEDERAL carries the GovCon lane, both of which DISCOVERY_QUALIFY's qualify step routes against
  "Cost Efficiency":    ["DOCTRINE", "ACCOUNTING_FINANCE", "ROI_GUIDE", "REVENUE_LAYER", "SALES_OBJECTIONS", "ACTIONS", "PLATFORM", "SEASON_ECONOMICS", "PROOF_ECONOMICS", "WARRANTY_CALLBACK", "DISCOVERY_QUALIFY"],// cost/margin/roi/job/lead + deal-closing action flow + the objection-handling playbook + season/weather cost bridge (gap #2) + what proof/testing is worth (proof pass) + "callback"/"rework" already alias here — a post-job complaint is a cost-and-reputation decision; "lead"/"customer"/"estimate" nodes live here too, which is why a brand-new lead/qualify/intake call routes to DISCOVERY_QUALIFY (the front of this same funnel, before REVENUE_LAYER/SALES_OBJECTIONS/ACTIONS apply)
  ...
  "Soil Stability":     ["SERVICE_ARCHITECTURE", "STEM_FOUNDATIONS", "GAP_BRIDGES", "CONCRETE_ENGINEERING", "SALES_OBJECTIONS", "WARRANTY_CALLBACK", "DISCOVERY_QUALIFY"], // concrete/lifting/void/soil/seawall + the deep geotech diagnostic block + the "just replace the slab" objection rebuttal + a concrete callback (resettled/cracked) needs CONCRETE_ENGINEERING's diagnosis alongside the triage playbook + a first-call concrete symptom (settled/still moving/drainage) needs DISCOVERY_QUALIFY's diagnostic questions before CONCRETE_ENGINEERING gets applied
...
const ALIAS = {
  ...
  // Discovery / lead-qualification (front of funnel, before SALES_OBJECTIONS/ACTIONS apply) → Cost
  // Efficiency (lead/customer/estimate nodes, carries DISCOVERY_QUALIFY) — "estimate" and "worth"/"quote"
  // already alias/exact-match above; a federal/GovCon-flavored intake still routes via federal/sam/veteran
  // (Business Licensing, also carries DISCOVERY_QUALIFY).
  discovery: ["lead", "customer"], qualify: ["lead", "customer"], qualifying: ["lead", "customer"],
  intake: ["lead", "customer"], prospect: ["lead", "customer"], consultation: ["lead", "estimate"],
  assessment: ["lead", "estimate"], call: ["lead", "customer"], request: ["estimate", "cost"], reach: ["lead", "customer"],
