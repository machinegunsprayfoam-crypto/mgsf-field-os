...
const LOCAL_TOOLS = [
  ...
  { id: "sitework-calc",    category: "estimator", kind: "compute", does: "sitework/paving takeoff (GC/sub-check): asphalt tonnage (area × thickness × density) OR aggregate base (cubic yards + tons) — volume is geometry, density is a typical ESTIMATE (verify the mix design / DOT spec); pavement section, subgrade, and drainage NOT designed. Surfaced: 811 locate, positive drainage, subgrade/base compaction, SWPPP if ≥1 acre, ADA where public. No pricing. (Site concrete = flatwork-calc.)", module: "api/sitework-calc.js" },
  ...
  { id: "portal",           category: "crm",       kind: "endpoint", does: "customer-facing portal — token-gated (HMAC per record), read-only safe view of one customer's job status/quote/docs-to-sign; owner generates the link (crew-gated), customer opens /portal.html?token=…; strict allowlist, never leaks cost/margin/internal data; gated on PORTAL_SECRET + KV", module: "api/portal.js" },
  ...
  { id: "ops-manager",      category: "pm",        kind: "compute", does: "the FOREMAN over the doer agents — reads all five doers' plans, ranks the crew's next actions by REVENUE/RISK (cash-in-hand AR > speed-to-lead > cooling bid > compliance-halt risk > throughput), surfaces what's blocked on a dark tool as escalations, derives each agent's Work-Hub station, and rolls ONE approve-queue + one-line summary up to the owner; pure/deterministic core, recommends+queues ONLY (outward still routes through the arms approval gate), never auto-sends, never fabricates", module: "api/ops-manager.js" },
  { id: "workhub",          category: "pm",        kind: "compute", does: "agent operating layer — a private WORKSTATION per doer-agent (status + current task + bounded scratchpad), a shared WORK HUB (full-roster board + open-until-acked job handoffs + activity feed), and an agent↔agent INTRANET (direct + broadcast messages, per-agent inbox). Internal only (outward still routes through the arms approval gate); complements the module-level gearbox. Pure core + gated KV persistence; never fabricates", module: "api/workhub.js" },
  { id: "pipeline",         category: "pm",        kind: "compute", does: "lead→estimate→job→actuals RAIL: normalize an estimate carrying its bid breakdown (BF/sets/hours/material/labor/cost/sell), advance a lead without regressing, convert a WON estimate into a job that CARRIES the bid forward (feeds yield-variance for real quoted-vs-actual margin), + railFor (where a customer sits + the ONE next action) and funnelHealth (the funnel-middle gap, quantified); pure, never fabricates (missing bid = null), no persistence — the app's sync layer persists", module: "api/pipeline.js" },
